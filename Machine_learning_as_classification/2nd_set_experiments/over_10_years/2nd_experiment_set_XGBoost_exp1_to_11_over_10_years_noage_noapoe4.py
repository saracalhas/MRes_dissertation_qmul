...
# ### This notebook contains experiment 1 to 11 of the XGBoost experiments to predict dementia over 10 years from baseline. This set excludes age and apoe4 alleles. This code was run via bash.
...
columnns_to_drop_2 = ['eid', 'APOE4', 'age_at_baseline','APOE4_alleles','ethnicity','LTFU', 'dementia_date', 'dementia_prevalence', 'dementia_incidence', 'Diagnosis_since_baseline', 'Diagnosis_years_since_baseline']
...
# Filter the dataset to include only "over 10 years group" group and controls
filtered_data = data[data['Years_Group'].isin([0, 4])].copy()


# In[12]:


# Drop the original Years_Group column
filtered_data.drop(columns=['Years_Group'], inplace=True)
...
print("Column names:", filtered_data.columns.tolist())
...
# ### Experiment 1: simpler settings to test the model
...
output_dir = 'ML_work_v11_XG_Boost_no_feature_selection_only_shap_over_10_years_years_noage_noapoe4'
...
# ### Experiment 2: Wider search + learning rate
...
# ### Experiment 3: Like experiment 2 + additional trees
...
shap.summary_plot(shap_values, features=X_train, feature_names=X_encoded.columns, plot_type="bar")
# Save the plot as a file in the output directory
output_path = os.path.join(output_dir, 'shap_values_bar_model3.png')
plt.savefig(output_path)
plt.close()  # Close the plot to free up memory
...
shap.summary_plot(shap_values, features=X_train, feature_names=X_encoded.columns)
# Save the plot as a file in the output directory
output_path = os.path.join(output_dir, 'shap_values_model3.png')
plt.savefig(output_path)
plt.close()  # Close the plot to free up memory
...
# ### Experiment 4: with ROC AUC and StratifiedKFold and gamma
...
# ### Experiment 11: like exp 10 and without oversampling the minority class
